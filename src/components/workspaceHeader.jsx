"use client"
import { favoriteAction } from "@/action/favorite-action";

export default function WorkspaceHeader({ id, title, isFavorite }) {
    const handleChange = async (e) => {
        const FORM = e.target.closest("form");
        const FORM_DATA = new FormData(FORM);
        await favoriteAction(FORM_DATA);
    }

    return (
        <form className="flex w-full justify-between items-center gap-3 py-3">
            <h3 className="text-charcoal text-2xl font-bold">{title}</h3>
            <input type="hidden" name="workspaceId" value={id} />
            <label htmlFor="ws-favorite" className="bg-slate-200 p-1 rounded-md">
                <input
                    type="checkbox"
                    name="favorite"
                    id="ws-favorite"
                ...
                />
                <svg className={"stroke-charcoal peer-checked:fill-amber-500 peer-checked:stroke-amber-500"} width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
